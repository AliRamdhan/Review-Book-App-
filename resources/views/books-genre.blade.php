        ...
        <div class="w-full px-8 text-xl font-bold py-4 border-b-2 ">
            @if ($genresBooks)
                @foreach ($genresBooks as $genreBook)
                    <div class="text-3xl font-semibold">{{ $genreBook->genreName }} Books</div>
                    <div class="text-lg font-medium ">Diskusikan buku dengan pengguna lain</div>
                @endforeach
            @endif
        </div>
        <div class="w-full px-16 mt-8">
            <div class="relative">
                        ...
                        @foreach ($genres as $genre)
                            <div class="swiper-slide w-1/6">
                                <a href="{{ route('book.genre', ['genreId' => $genre->genreName]) }}">
                                    <div
                                        class="text-base font-bold text-gray-600 text-center rounded border border-black">
                    ...
                    @if ($genresBooks)
                        @foreach ($genresBooks as $genreBook)
                            @foreach ($genreBook->books as $book)
                                <div
                                    class="h-[220px] flex justify-center items-center border border-black rounded-r-xl rounded-black">
                                    ...
                                    <div class="w-full flex flex-col justify-center gap-1 px-2">
                                        ...
                                        <div class="font-medium"
                                            ...
                                            {{ $book->bookSynopsis }} Lorem ipsum dolor sit amet, consectetur
                                            adipisicing elit.
                                            Iste velit accusantium nihil minus aut iure quaerat eius distinctio
                                            praesentium
                                            alias natus exercitationem cupiditate quo laborum, odio adipisci fugit,
                                            ducimus
                                            ipsam.</div>
                                        <div>
